...
4. **部屋配置**: マスク内に 2x2〜4x3 の小部屋をランダム配置（重複禁止、壁1枚隣接は許可）
5. **島ごと DFS**: 各島で独立に迷路を生成。部屋内ノードは pre-carved として扱い、廊下と自然接続
6. **通路拡張**: 内壁の 15-25% を候補に壁除去を試行。flood fill で連続空間 ≤ 12 マスの場合のみ確定
7. **ドア配置**: 部屋外周の壁セルで反対側が廊下のものを候補にドアを配置。部屋サイズで種類決定。廊下ハブ構造を実現
8. **階段配置**: 廊下にのみ配置（部屋内は除外）。各島に階段を配置し、異なる階の異なる島に遷移させることで迷子感を演出
9. **レイキャスティング**: VOID に ray が到達すると `Some(RayHit{tile: TILE_VOID})` を返す。壁描画なし・床天井も抑制され、列全体が黒に
